...
public class ItemSwitcherScreen extends ComponentScreen {
	...
	public void renderComponents(DrawContext context, int mouseX, int mouseY) {
		lastMouseX = mouseX;
		lastMouseY = mouseY;
		super.renderComponents(context, mouseX, mouseY);
	}
	...
	protected Component createComponent(InventoryItem item) {
		return new CustomInventoryItemComponent(ItemSwitcherScreen.this, item) {
			...
			@Override
			public void render(DrawContext drawContext, PositionedRectangle renderBounds, int x, int y, int mouseX, int mouseY) {
				if (isHovered()) {
					RenderUtil.drawCircle(drawContext, x + getBoundingBox().width() / 2f, y + getBoundingBox().height() / 2f, 11, new Color(150, 150, 150, 170).color());
				} else {
					RenderUtil.drawCircle(drawContext, x + getBoundingBox().width() / 2f, y + getBoundingBox().height() / 2f, 11, new Color(20, 20, 20, 170).color());
				}
				super.render(drawContext, renderBounds, x, y, mouseX, mouseY);
			}
			...
			public void postRender(DrawContext context, PositionedRectangle renderBounds, int x, int y, int mouseX, int mouseY) {
				if (isHovered()) {
					...
					context.drawCenteredTextWithShadow(client.textRenderer, item.getStack().getName(), x + rect.width() / 2, y - 10, -1);
					context.getMatrices().translate(0, 0, -500);
				}
			}
		};
