	...
	public List<InventoryItem> search(List<InventoryItem> items) {
		if (parameters.containsKey(SearchFilter.NAME)) {
			...
			items = items.stream().filter(item -> {
				...
				if (Registries.ITEM.getId(item.getStack().getItem()).toString().toLowerCase(Locale.ROOT).contains(query)) {
					return true;
				}
				return false;
			}).toList();
		...
		if (parameters.containsKey(SearchFilter.GROUP)) {
			...
			items = items.stream().filter(item -> {
				return item.getGroups().stream().anyMatch(tag -> tag.getDisplayName().getString().toLowerCase(Locale.ROOT).contains(query));
			}).toList();
		}
		return items;
	}
